...
def test_simple() -> None:
    """A module provided by a workspace sibling that is not declared fires DEP006."""
    module = ModuleBuilder("bar", set(), frozenset()).build()

    issues = DEP006MissingWorkspaceDependenciesFinder(
...
def test_not_a_workspace_sibling() -> None:
    """A module that is not a workspace sibling is not flagged by DEP006."""
    module = ModuleBuilder("bar", set(), frozenset()).build()
...
def test_provided_by_dependency() -> None:
    """A workspace sibling that IS declared as a dependency does not fire DEP006."""
    module = ModuleBuilder("bar", set(), frozenset()).build()
